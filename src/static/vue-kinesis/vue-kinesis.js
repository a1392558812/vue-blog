    ...
    transformAudio() {
      ...
      switch (transformType) {
        ...
        case 'scale':
          ...
          if (this.maxScale !== -1 && amplitude > this.maxScale) {
            amplitude = amplitude / this.maxScale
          }
          if (this.minScale !== -1 && amplitude < this.minScale) {
            amplitude = amplitude / this.minScale
          }
          break
      } // eslint-disable-next-line consistent-return

      this.$emit('up-context', this.context)
      return {
        transform
      }
    }
...
function render(_ctx, _cache, $props, $setup, $data, $options) {
  return (
    ...
    createBlock(
      ...
      {
        style: { ...$options.transform, ...$options.transformParameters }
      },
      {
...
function mouseMovement(action) {
  ...
  return { ...getCoordinates(mouseMovementX, mouseMovementY), target }
}
  ...
  methods: {
    ...
    handleMovement: throttle(function (event) {
      ...
      if (this.event === 'move' && this.isMoving && !isTouch()) {
        ...
        (this.event === 'orientation' || (this.event === 'move' && isTouch())) &&
        isInViewport
      ) {
...
function render$1(_ctx, _cache, $props, $setup, $data, $options) {
  return (
    ...
    createBlock(
      ...
      8,
      ['onMousemove', 'onMouseenter', 'onMouseleave', 'style']
    )
  )
}
...
function render$2(_ctx, _cache, $props, $setup, $data, $options) {
  return (
    ...
    createBlock(
      resolveDynamicComponent($props.tag),
      {
        style: { ...$data.transform, ...$options.transformParameters }
      },
      {
        default: withCtx(() => [renderSlot(_ctx.$slots, 'default')]),
        _: 3
...
var transformMixin = {
  methods: {
    ...
    scaleMovement(x, y) {
      const { type } = this
      let movement = (Math.sign(this.strength) * (Math.abs(x) + Math.abs(y))) / 10 + 1
      if (this.maxScale !== -1 && movement > this.maxScale) {
        movement = movement / this.maxScale
      }
      if (this.minScale !== -1 && movement < this.minScale) {
        movement = movement / this.minScale
      }
      return `scale3d(${type === 'scaleX' || type === 'scale' ? movement : 1},
            ${type === 'scaleY' || type === 'scale' ? movement : 1},
            1)`
...
function elementMovement(action) {
  const { y, x, target, originX = 50, strength = 10, event = null } = action
  ...
  return { ...getCoordinates(movementX, movementY), target }
}
...
function cyclicMovement(cycleData) {
  const { referencePosition, elementPosition, spanningRange, cycles } = cycleData
...
var script$3 = {
  ...
  methods: {
    transformMovement() {
      ...
      transformType =
        transformType === 'scaleX' || transformType === 'scaleY' ? 'scale' : transformType
  ...
  methods: {
    ...
    transformBehavior(shape) {
      ...
      if (this.cycle <= 0) {
        ...
        const { x, y, width, height } = this.getCycleMovement(
          0,
          0,
          window.innerWidth,
          window.innerHeight,
