...
export function getRandText(text) {
...
export function initModel(waifuPath, type) {
  ...
  live2d_settings.waifuTipsSize = isArray(live2d_settings.waifuTipsSize)
    ? live2d_settings.waifuTipsSize
    : live2d_settings.waifuTipsSize.split('x')
  live2d_settings.waifuEdgeSide = isArray(live2d_settings.waifuEdgeSide)
    ? live2d_settings.waifuEdgeSide
    : live2d_settings.waifuEdgeSide.split(':')
  ...
  try {
    ...
    console.log(err, '[Error] JQuery UI is not defined.')
  }
...
export function loadTipsMessage(result) {
  ...
  function loadOtherModel() {
    const modelId =
      +modelStorageGetItem('modelId') === modelList.length - 1
        ? 0
        : +modelStorageGetItem('modelId') + 1
    loadModel(modelId)
  ...
  $('.waifu-tool .fui-volume').click(function () {
    ...
    showHitokoto(
      `<a href="${url}#/music">┭┮﹏┭┮~~555，实在不想再写功能了,点击这里-→前往我之前写的音乐demo里欣赏吧</a>`,
      15000
    )
  })
}
