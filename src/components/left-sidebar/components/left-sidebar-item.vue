...
export default {
  ...
  setup(props, { emit }) {
    ...
    const renderList = computed(() =>
      rowDetails.value && rowDetails.value.children ? rowDetails.value.children : []
    )
    const tag = computed(() => (renderList.value.length ? 'div' : 'a'))
    const hrefMap = computed(() =>
      renderList.value.length ? {} : { href: `/#/?indexPage=${rowDetails.value.indexPage}` }
    )
  ...
  render() {
    ...
    return this.rowDetails && this.rowDetails.name ? (
      ...
      >
        ...
        <div onClick={this.onRowClick} class="cell-item-title flex">
          ...
          {this.renderList.length ? (
            ...
            >
              <path
                d="M349.184 208.384l346.624 261.632c22.528 16.896 27.136 49.152 10.24 71.68-3.072 3.584-6.144 7.168-10.24 10.24l-346.624 261.632c-11.264 8.704-27.136 6.144-35.84-5.12-3.584-4.608-5.12-9.728-5.12-15.36V228.864c0-14.336 11.264-25.6 25.6-25.6 5.12 0 10.752 2.048 15.36 5.12z"
                p-id="34098"
              ></path>
            </svg>
          ) : null}
