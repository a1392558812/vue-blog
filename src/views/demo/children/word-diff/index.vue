<template>
  <div
    class="diff-warp min-h-screen bg-gradient-to-br from-indigo-50 to-purple-50 p-4 overflow-auto"
  >
    <!-- 操作提示 -->
    <div
    ...
    >
      <div class="text-gray-600 text-sm">
        ...
        <div>
          •
          <span class="inline-block px-2 py-0.5 bg-green-100 text-green-800 rounded">绿色</span>
          表示在第二段新增的内容
        </div>
        <div>
          •
          <span class="inline-block px-2 py-0.5 bg-red-100 text-red-800 rounded">红色</span>
          表示在第一段删除的内容
        </div>
      </div>
    </div>
    ...
    <div class="w-[1500px] mx-auto">
      <div class="flex flex-col gap-4">
        <div class="grid grid-cols-3 gap-4">
          <!-- 左侧：str1 -->
          <div
            class="bg-white rounded-xl shadow-md p-4 transition-all duration-300 hover:shadow-lg"
          >
            ...
            ></textarea>
            <div
              class="text-center py-2 font-medium text-gray-700 border-b-2 border-indigo-100 mt-3 mb-2"
            >
              预览【 ↓ 】：
            </div>
          ...
          >
            ...
            ></textarea>
            <div
              class="text-center py-2 font-medium text-gray-700 border-b-2 border-purple-100 mt-3 mb-2"
            >
              预览【 ↓ 】：
            </div>
            <div class="p-3 bg-gray-50 rounded-lg overflow-auto max-h-[350px]">
    ...
    const str3 = computed(() =>
      [''].concat(diffChars(str1.value, str2.value)).reduce((total, next) => {
        ...
        if (next.added) {
          result =
            result +
            `<span style="background: #dcfce7; color: #166534; padding: 0.5px 2px; border-radius: 2px;">${next.value.replaceAll(' ', '<span>&#8194;</span>')}</span>`
        } else if (next.removed) {
          result =
            result +
            `<span style="background: #fee2e2; color: #991b1b; padding: 0.5px 2px; border-radius: 2px;">${next.value.replaceAll(' ', '<span>&#8194;</span>')}</span>`
        } else {
