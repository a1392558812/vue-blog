<template>
    <div class="diff-warp overflow-y-auto">
        <div class="flex justify-content-space-between">
            ...
            <div class="flex-1 flex-direction-column align-items-center justify-content-center" style="border-left: 1px solid #000; border-right: 1px solid #000;">
...
`)
    ...
    const str3 = computed(() => ([''].concat(diffChars(str1.value, str2.value))).reduce((total, next) => {
      let result = total
      if (next.added) {
        result = result + `<span style="background: green;">${next.value.replaceAll(' ', '<span>&#8194;</span>')}</span>`
      } else if (next.removed) {
        result = result + `<span style="background: red">${next.value.replaceAll(' ', '<span>&#8194;</span>')}</span>`
      } else {
