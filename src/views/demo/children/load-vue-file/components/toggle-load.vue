...
export default {
  ...
  setup() {
    ...
    const inputValue1 = Vue.ref(
      '<template><div class="test-a">a组件字符串内容</div></template><style scoped>.test-a{ color: red }</style>'
    )
    const inputValue2 = Vue.ref(
      '<template><div class="test-b">b组件字符串内容</div></template><style scoped>.test-b{ color: green }</style>'
    )
