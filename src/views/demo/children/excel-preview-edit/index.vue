<template>
  <div class="overflow-auto excel-preview-edit">
    <div style="margin:20px" class="flex flex-direction-column">
      <div>
        <div class="loading-status" v-if="loadScript === 0 || loadCss === 0">加载excel解析组件中...</div>
        <excel-lucky-sheet class="excel-lucky-sheet" v-if="loadScript === 1 && loadCss === 1"></excel-lucky-sheet>
...
export default {
  ...
  setup () {
    ...
    const cssListLoad = []
    asyncLoadJs(`${baseUrl}demo-static/excel-preview-edit/plugins/js/plugin.js`)
      .then(_ => {
        asyncLoadJs(`${baseUrl}demo-static/excel-preview-edit/luckysheet.umd.js`).then(_ => {
          loadScript.value = 1
        }, e => {
          loadScript.value = -1
        })
    ...
    ;['plugins/css/pluginsCss.css', 'plugins/plugins.css', 'css/luckysheet.css', 'assets/iconfont/iconfont.css'].forEach(item => {
      cssListLoad.push(asyncLoadCss(`${baseUrl}demo-static/excel-preview-edit/${item}`))
    })
    Promise.all(cssListLoad).then(_ => {
      loadCss.value = 1
    }, e => {
      loadCss.value = -1
    })
