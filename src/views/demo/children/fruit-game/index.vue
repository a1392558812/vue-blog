  ...
  <div
    ...
    <div v-if="gameState === 'start'"
      ...
      <button @click="startGame"
        class="start-button px-8 py-[0.8rem] text-[1.2rem] bg-[#ff6b6b] text-[white] border-none rounded-[30px] cursor-pointer [transition:all_0.3s_ease] [box-shadow:0_4px_15px_rgba(0,_0,_0,_0.2)]">开始游戏</button>
    ...
    <div v-else-if="gameState === 'playing'" class="w-full h-full relative">
      <div
        ...
        <div class="flex items-center gap-[0.5rem] text-[1.1rem] [text-shadow:0_1px_3px_rgba(0,_0,_0,_0.3)]">
          <span>⭐</span>
          <span>分数: {{ score }}</span>
        </div>
        <div class="flex items-center gap-[0.2rem] text-[1.1rem] [text-shadow:0_1px_3px_rgba(0,_0,_0,_0.3)]">
      ...
      <!-- 游戏区域 -->
      <div class="game-area w-full h-full relative overflow-hidden" @mousedown="startSwipe" @mousemove="handleSwipe"
        @mouseup="endSwipe" @touchstart="startSwipe" @touchmove="handleSwipe" @touchend="endSwipe">
        <!-- 水果和炸弹 -->
        <div v-for="(item, index) in items" :key="item.id" :class="['game-item', item.type]" :style="{
          left: `${item.x}%`,
          top: `${item.y}%`,
          ...
          <!-- 切开效果 -->
          <div v-if="item.sliced" class="absolute w-full h-full flex items-center justify-center select-none">
    ...
    <div v-else-if="gameState === 'end'"
      ...
      <button @click="restartGame"
        class="restart-button px-8 py-[0.8rem] text-[1.2rem] bg-[#4ecdc4] text-[white] border-none rounded-[30px] cursor-pointer [transition:all_0.3s_ease] [box-shadow:0_4px_15px_rgba(0,_0,_0,_0.2)]">再玩一次</button>
    </div>
  </div>
</template>
...
const spawnItem = () => {
  ...
  for (let i = 0; i < level.value; i++) {
    if (items.value.length + i < maxItems.value) { // 避免超出最大物品数
